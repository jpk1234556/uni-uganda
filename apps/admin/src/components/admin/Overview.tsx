...
import { Loader2, AlertCircle, Building, Users, Calendar, ArrowRight, ExternalLink } from "lucide-react";
  ...
  return (
    ...
    >
      <div>
        ...
        <p className="text-slate-500 mt-2 text-lg">Platform metrics and system status at a glance.</p>
      </div>
        ...
        <motion.div variants={itemVariants} className="xl:col-span-2">
          <Card className="shadow-sm border-slate-200 overflow-hidden h-full">
            <CardHeader className="bg-slate-50/50 border-b border-slate-100 flex flex-row items-center justify-between py-5">
              <CardTitle className="text-lg text-slate-800">Recent Hostel Signups</CardTitle>
              <Link to="/admin/hostels" className="text-sm font-medium text-indigo-600 hover:text-indigo-700 flex items-center gap-1 group">
            ...
            <CardContent className="p-0">
              {isLoading ? (
                <div className="flex flex-col items-center justify-center py-12 text-center bg-white">
                  ...
                  <span className="text-slate-400 font-medium">Loading network data...</span>
                </div>
              ) : recentHostels.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center bg-white">
                  <span className="text-slate-400">No properties in the system yet.</span>
                </div>
              ) : (
                <div className="divide-y divide-slate-100">
                  {recentHostels.map((hostel) => (
                    <div key={hostel.id} className="p-5 flex items-center justify-between hover:bg-slate-50 transition-colors">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">Owner: {hostel.users ? `${hostel.users.first_name} ${hostel.users.last_name}` : 'Unknown'}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <span className={`px-2.5 py-1 text-xs font-semibold rounded-full ${hostel.status === 'approved' ? 'bg-emerald-100 text-emerald-700' : hostel.status === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-rose-100 text-rose-700'}`}>
            ...
            <CardContent className="p-5">
              {metrics.pendingVerifications > 0 ? (
                <div className="p-4 bg-white rounded-xl border border-amber-200 shadow-sm">
                  <p className="font-bold text-amber-900 text-lg">{metrics.pendingVerifications} Pending Hostels</p>
                  <p className="text-sm text-amber-700/80 mt-1 mb-4">Properties are waiting for your verification before they go live on the student portal.</p>
                  <Link to="/admin/hostels" className="inline-flex w-full items-center justify-center px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white text-sm font-medium rounded-lg transition-colors shadow-sm">
                ...
                <div className="p-6 text-center bg-white rounded-xl border border-emerald-100 shadow-sm">
                  ...
                  <p className="text-sm text-emerald-600/80 mt-1">No pending verifications.</p>
                </div>
              )}
            </CardContent>
          </Card>
...
function StatsCard({ title, value, loading, icon, gradient, delay }: { title: string; value: string | number; loading: boolean; icon: React.ReactNode; gradient: string; delay: number }) {
  return (
    <motion.div
